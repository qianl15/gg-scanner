...
if __name__ == "__main__":
  ...
  names = [tt.name for op in graph.get_operations() for tt in op.values()]
#  pp.pprint(names);
...
#    })
    results = sess.run(output_tensor, feed_dict={input_tensor: t})
  results = np.squeeze(results)

  top_k = results.argsort()[-5:][::-1]
  labels = load_labels(label_file)
  for i in top_k:
    print('{} ({:d}): {:.7f}'.format(labels[i], i, results[i]))
